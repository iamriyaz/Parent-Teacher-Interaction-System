...
define('mysqli_NOT_EXISTS', create_error("Your PHP server doesn't have the mysqli module loaded or you can't use the mysqli_(p)connect functions."));
...
if (!function_exists('mysqli_connect') || !function_exists('mysqli_connect') || !extension_loaded('mysqli')){
	echo mysqli_NOT_EXISTS;
	die();
}
...
/*
 // The 'var' keyword is deprecated in PHP5 ... we will define these variables at runtime.
  var $isOpen;
	...
	var $error;
*/
	function mysqliConnection($ConnectionString, $Timeout, $Host, $DB, $UID, $Pwd)
	{
		$this->isOpen = false;
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId) && is_resource($this->connectionId)){
			...
			//http://dev.mysqli.com/doc/refman/5.0/en/legal-names.html
	...
	function GetColumnsOfTable($TableName)
	{
		...
		if ($result)
		{
			...
			while ($row = mysqli_fetch_array($result, mysqli_ASSOC))
			{
				...
				// mysqli sets nullable to "YES" or "", so we need to set "NO"
				$null = $row["Null"];
				if ($null == "")
					$null = "NO";
	...
	*/
	function HandleException()
	{
		...
		$this->error = create_error(' mysqli Error#: '. ((int)mysqli_errno()) . "\n\n".mysqli_error());
